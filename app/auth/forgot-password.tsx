...
import { Description, Input, Label, TextField, useThemeColor } from "heroui-native";
...
export default function ForgotPassword() {
  const [foreground, danger, accent, accentForeground, success] = useThemeColor(["foreground", "danger", "accent", "accent-foreground", "success"]);
          ...
          >
            <View className="flex-1 justify-center">
              <View className="gap-y-2 items-center">
                ...
                <Text className="text-xl font-semibold text-center mt-4" style={{ color: foreground }}>
                  Check Your Email
                </Text>
                <Text className="text-sm text-center opacity-70 mt-2" style={{ color: foreground }}>
                ...
                >
                  <Text style={{ color: accentForeground }}>Send Another Email</Text>
                </Pressable>
              </View>

              {/* Login Link */}
              <View className="mt-8 flex-row justify-center items-center">
                <Text className="text-sm opacity-70" style={{ color: foreground }}>
          ...
          <View className="flex-1 justify-center">
            ...
            <View className="gap-y-2">
                    ...
                    <TextField className="w-full">
                      ...
                      <View style={{ position: "relative" }}>
                        ...
                        <View style={{ position: "absolute", left: 10, top: 12 }}>
                          <Ionicons name="mail-outline" size={20} color={foreground} style={{ opacity: 0.7 }} />
                        </View>
                      </View>
                      {errors.email && (
                        <Description style={{ color: danger }}>{errors.email.message}</Description>
                      )}
                    </TextField>
                  )}
                />
              ...
              <Pressable
                ...
                disabled={isLoading}
              >
                <Text style={{ color: accentForeground }}>{isLoading ? "Sending..." : "Send Reset Email"}</Text>
              </Pressable>
            </View>
