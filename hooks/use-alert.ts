...
export default function useAlert(): UseAlertReturn {
  const [successColor, dangerColor, warningColor, accentColor] = useThemeColor([
    "success",
    "danger",
    "warning",
    "accent",
  ...
  const getAlertColor = (type: NotificationType): string => {
    switch (type) {
      case "success":
        return successColor || "#17C964";
      case "error":
        return dangerColor || "#F31260";
      case "warning":
        return warningColor || "#F5A524";
      case "info":
      default:
        return accentColor || "#006FEE";
    }
  };
